      ...
      <div className="flex flex-1 flex-col px-7 pb-8 pt-10">
        ...
        <ul className="mt-auto space-y-2 border-t border-hairline pt-5">
          {service.items.slice(0, 3).map((item) => (
            <li key={item} className="flex gap-3 text-[13.5px] leading-snug text-ink">
...
export default function ServicesSection() {
  ...
  useLayoutEffect(() => {
    ...
    matchMedia.add('(min-width: 1024px)', () => {
      ...
      const pinDistance = () => {
        ...
        return Math.max(window.innerHeight * 1.15, distance + window.innerHeight * 0.75)
      }
      ...
      gsap.set(track, {
        x:
          -measureDistance() *
          (maxActiveRef.current === 0 ? 0 : clampedActiveRef.current / maxActiveRef.current),
      })
  ...
  return (
    <section ref={sectionRef} id="servicios" className="overflow-hidden bg-bone py-24 md:py-32">
      <div className="mx-auto grid w-full max-w-[1440px] grid-cols-1 gap-12 px-6 md:px-10 lg:grid-cols-12 lg:gap-16">
        ...
        <div className="lg:col-span-4">
          ...
          <div className="mt-9 flex flex-wrap items-center gap-4">
            <PillButton as="a" href="#contacto">Hablemos</PillButton>
            <div className="flex items-center gap-2">
              <ArrowButton direction="left" onClick={() => go(-1)} disabled={clampedActive === 0} />
              <ArrowButton direction="right" filled onClick={() => go(1)} disabled={clampedActive >= maxActive} />
            </div>
          </div>
          ...
          <div className="mt-10 max-w-[280px]">
            ...
            <div className="mt-3 flex items-center justify-between font-mono text-[10px] uppercase tracking-[0.22em] text-ink-mute">
              <span>{counter}</span>
              <span>Auto-prestado</span>
            </div>
          </div>
        </div>

        {/* RIGHT — sliding track */}
        <div ref={trackContainerRef} className="min-w-0 overflow-hidden lg:col-span-8">
          <div
            ref={trackRef}
            className="flex items-stretch gap-6 will-change-transform"
            style={{
              transform: scrollMode ? undefined : `translate3d(${offset}px, 0, 0)`,
              transition: scrollMode ? 'none' : `transform ${DURATION}ms ${EASE}`,
              minHeight: 540,
          ...
          >
            {services.map((service, index) => (
              ...
              >
                <ServiceCard
                  service={service}
                  isActive={index >= clampedActive && index < clampedActive + visibleCount}
                />
              </div>
            ))}
          </div>
